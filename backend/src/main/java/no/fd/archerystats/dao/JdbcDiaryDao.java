...
public class JdbcDiaryDao implements DiaryDao {
    ...
    public List<Diary> findDiary(String userId, Date fromDate, Date toDate) {
        ...
        return this.jdbcTemplate.query("select * from archerystats_v1.diary where id_user = ? and date >= ? and date <= ?", new Object[]{userId, fromDate, toDate}, diaryRowMapper);
    }

    public List<Diary> findDiary(String userId, Date fromDate, Date toDate, Integer spt) {
        LOGGER.info("Find diary");
        return this.jdbcTemplate.query("select * from archerystats_v1.diary where id_user = ? and date >= ? and date <= ? and spt = ?", new Object[]{userId, fromDate, toDate, spt}, diaryRowMapper);
    }
