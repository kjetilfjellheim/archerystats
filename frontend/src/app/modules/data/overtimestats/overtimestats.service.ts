...
export class OvertimeStatsService {
    ...
    getStatistics(totalDateRange: Date[], mindistance: number, maxdistance: number) : Observable<ValueByDate[]> {
        return this.http.get("/archerystats/request/statistics/overtime?mindistance=" + mindistance + "&maxdistance=" + maxdistance + "&fromDate=" + totalDateRange[0].toISOString().substring(0, 10) + "&toDate=" + totalDateRange[1].toISOString().substring(0, 10))
